...
depois que ela estiver aberta, está no [manual](manual.md) — dentro da IDE, em Ajuda › Manual.
...
4. confirme que `README.md`, `DocsPublic/manual.md` e este tutorial representam a
...
- copia `dist/instalar-kinein-vectis.sh`, responsável pelo único atalho do
  usuário;
- copia o `DocsPublic/tutorial.md` vigente para `dist/`, sem depender da árvore-fonte na
  entrega.
...
- `DocsPublic/manual.md`: ensina exclusivamente a usar a IDE;
- `DocsPublic/tutorial.md`: ensina distribuição, instalação, atualização e geração do
  executável portátil.
