...
[ ] 5. Entrada no registro-de-componentes-abertos.json (pin + checksum + escopo +
       verificação)
...
| Documento | Assunto |
| --- | --- |
| [40-ros2-ecossistema-e-integracao.md](40-ros2-ecossistema-e-integracao.md) | **Avaliação ROS 2 para C/C++/Rust/Python e embarcados** (2026-09-24 a 2026-09-25): ferramentas e plugins abertos, licenças e manutenção, ambientes/overlays, colcon, launch/debug, QoS, bags, micro-ROS e simulação; reuso IPC, lacunas e critérios de prova para versão futura, fora da 0.3. Candidatas, não adotadas |
...
| [39-toolchains-por-alvo.md](39-toolchains-por-alvo.md) | **Toolchains por alvo, com credibilidade conferida na fonte** (2026-09-12): Arm GNU Toolchain 15.2.rel1, xPack, Espressif `xtensa-esp-elf`/`riscv32-esp-elf` (unificadas — o nome por chip está desatualizado), Arm Toolchain for Embedded 23.1.0 (sucessor do LLVM Embedded), Bootlin, Yocto/Buildroot, Zephyr SDK, Rust por `rustup target add`; o problema do **sysroot** dos compiladores de distro (medido: vazio no Fedora); onde a IDE procura além do PATH; a regra do provedor de instalação (clique, checksum, pasta da IDE) |
...
| Tree-sitter (+ gramáticas C/C++/Rust) | A | Sintaxe incremental local | [ADR-0002](../decisoes-adr/ADR-0002-tree-sitter-syntax-foundation.md) |
