...
> em `docsprivate/ContextoIA.md`, e então removido daqui.
>
> **Estado implementado: o CÓDIGO — mede-se, não se lê.** O `docsprivate/ContextoIA.md` é
> LOG datado e não responde "o que existe hoje" (rebaixado em 2026-07-17; ver
> `docs/README.md`). Mapa: `docsprivate/GUIAIA.md`. Histórico: Git. Protocolo `0.61.0`.
>
...
> gesto. Não é log — log é o `docsprivate/ContextoIA.md`, e ele não manda em nada.
...
está lá dentro. Print de referência: `docsprivate/imagens/prints/` (2026-07-16).
...
```text
Leia docsprivate/AGENTS.md e retome pelo marcador REENTRADA-KV em docsprivate/PONTO_ATUAL.md. Siga as
...
1. ler `docsprivate/AGENTS.md` e a documentação obrigatória indicada nele, sem devolver
   uma recapitulação extensa ao usuário;
2. localizar primeiro o marcador `REENTRADA-KV` abaixo e confrontá-lo com
   `docsprivate/ContextoIA.md` + código apenas onde houver divergência;
...
- A auditoria seguinte encontrou `target/release/kinein-core` anterior a
  ...
  `docsprivate/ContextoIA.md`. `dist/` permaneceu intocado.
...
`docs/roadmaps/20` e `docsprivate/ContextoIA.md`.
...
bloqueante; a imagem de referência é `docsprivate/imagens/bugs/ReformularBarra.png`.
...
"para nao ficar meio renomeado". NAO renomeados de proposito: `docsprivate/ContextoIA.md`,
`docsprivate/diario/` e o ADR-0004, que sao LOG do que foi decidido quando o nome era
...
publicação. A política de tom já existe em `docsprivate/PLANO_ORGANIZACAO_E_HANDOFF.md` §6,
...
**O conflito com a política do projeto.** O `docsprivate/AGENTS.md` proíbe "escolher revisão
...
- **Reorganização da documentação (concluída, sem commit):** os `.md` técnicos
  ...
  (faixa pessoal, não publicar): `docsprivate/PLANO_ORGANIZACAO_E_HANDOFF.md`.
...
`docsprivate/ContextoIA.md`. A entrada única continua sendo `scripts/medir-performance.sh` +
...
- **remake de UI/UX do shell, com o IntelliJ IDEA Community como layout-base.**
  ...
  `docsprivate/AGENTS.md`/roadmap de adaptação — estudar revisão atual, registrar
  ...
  Imagem de referência do autor para a barra: `docsprivate/imagens/bugs/ReformularBarra.png`.
...
1. Ler `docsprivate/GUIAIA.md` e as fontes do domínio antes de editar.
...
9. Registrar conclusão em `docsprivate/ContextoIA.md` e no doc do domínio; remover o item
...
- estado técnico, decisões e checkpoints: `docsprivate/ContextoIA.md`;
...
- plano de daily driver e longo prazo: `docsprivate/diario/18-daily-driver-plan.md` e
