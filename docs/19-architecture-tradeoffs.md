...
> `docs/ARCHITECTURE.md`; o *o quê/quando* em `docs/18-daily-driver-plan.md`
> e `docs/BACKEND_TO_UI_UX_ROADMAP.md`; o visual-alvo em `docs/specs/`
...
2026-07-09 a pedido do usuário.)

## Requisitos funcionais (resumo por área)

Detalhe e sequência vivem em `docs/18` (marcos M0–M4) e no roadmap; aqui só
...
RNF7 Performance percebida: startup rápido (LSP/ferramentas sob demanda,
     ...
     Orçamento formal de medição entra no M4 (docs/18).
...
- **Custo aceito:** serialização em cada chamada; dois toolchains e dois
  builds; contrato para manter (`docs/03` + schemas).
...
controllers; eventos em `ipc/*`; `CoreClient` fachada única (docs/17).
...
Registrado em detalhe em `docs/18` (design M1.1). Resumo do trade-off:
...
- **Revisitar:** relaxar exige motivo registrado (AGENTS.md); a direção
  preferida é subir degraus (docs/18, escada de rigor).
...
- **Custo aceito:** nenhum cliente externo pode depender do protocolo antes
  do V1; toda mudança exige docs/03 + schemas sincronizados no mesmo commit.
...
`docs/20-ui-spec-convergence-plan.md`.
...
- **Ganho:** IDE utilizável durante toda a transição (dogfooding contínuo,
  ...
  visual/lógica já paga em docs/17; UI nova (Main Toolbar, KV Context,
...
- **Revisitar:** gatilhos objetivos na seção "Gatilho de remake" de
  docs/20 (acoplamento visual pior que o mapeado na C1; estouro de 2x em
