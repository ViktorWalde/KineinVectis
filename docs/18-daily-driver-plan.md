...
marcos — regras, inventário e ordem em `docs/20-ui-spec-convergence-plan.md`
(decisão D12 em docs/19). Em resumo: C0 (auditoria) fecha com o M1; C1
...
Estruturação feita antes do código, seguindo `docs/ARCHITECTURE.md`.
...
**Fluxo (guardrails de docs/17 preservados):**
...
[feito] docs/03-ipc-protocol.md                   contrato documentado
```
...
- Espans ficam ancorados por linha entre uma edição e a resposta do refresh
  ...
  é complexidade de editor que não paga o custo hoje (ver docs/19).
```
...
[feito] docs/18 + ContextoIA.md         notas antigas corrigidas, fatia
                                        marcada
```
...
empírica (limitação registrada em docs/19).
...
- Validação: fatia sem superfície testável no core (lógica QML — gap D9 de
  docs/19). Gate completo + smoke offscreen + uso real pelo usuário.
```
...
docs/20) vem antes de qualquer UI nova deste marco. Cada fatia ganha sua
...
[feito] docs/03 + MANUAL + ContextoIA           contrato/uso/estado
```
...
[feito] docs/03 + ContextoIA                   contrato/estado
```
...
clique/atalho fica com o usuario (R7 de docs/20) — o pipeline por baixo
...
repo" (histórico de commits com diff por clique). Régua docs/21:
...
Primeira fatia da Trilha T (docs/21): micro-fatia de alto valor diário no
...
Trilha T, a maior lacuna diária de editor (docs/21). Régua explícita do
...
- Store por arquivo num CONTROLLER NOVO dedicado (DiagnosticsController,
  guardrail docs/17: concern próprio, não incha o JobsController).
...
- Pendente do usuário: validação visual (R7 de docs/20 — squiggle e
...
Primeira fatia do M4 (docs/21). É a fatia que destrava format-on-save,
...
**Decisões (e porquês, honrando docs/21 M4.1):**
...
- Settings são TIPADOS (docs/21: sem engine genérica de formulário).
...
- Primeiros consumidores REAIS (3 dos 4 de docs/21; o 4º registrado):
...
Fecha (parte A) o M4.3 do docs/21. Hoje se o `kinein-core` morre
...
sem reiniciar a janela (critério de aceite do docs/21).
...
Design, problemas e status vivem em **`docs/23`** (a fatia é grande e o
...
Fecha (junto de M4.2) o M4 do docs/21. Um setting (extensão da M4.1) que
...
Fecha (junto de M4.5) o miolo do M4 do docs/21. NÃO é uma fatia de
...
quiser.** Máquina de referência (docs/14): AMD Ryzen 7 7735HS (16
...
PROCEDIMENTO MANUAL documentado (o docs/21 já permite) — não invento
...
- (B) Core workspace.open: round-trip stdio no PRÓPRIO repo Kinein (o
  "workspace grande" do docs/21). Mede do spawn até a resposta do
...
- Latência de digitação (item c do docs/21): PROCEDIMENTO MANUAL — abrir
...
- Onde mora o orçamento: TABELA no docs/21 (fonte única do orçamento) +
  esta nota registra a 1ª medição. Regressão de orçamento = bug (fatia
  de correção antes de feature nova), como manda o docs/21.
```
...
scripts/medir-performance.sh (novo); docs/21 (tabela de orçamento com
números reais); docs/14 (documenta o script ao lado do verificar.sh);
docs/18 (esta nota); ContextoIA (M4 quase fechado).
...
orçamento em docs/21; script documentado em docs/14. Descoberta: `fs.read`
...
docs). Latência de digitação ficou como métrica MANUAL (docs/21).
...
- git.stage { paths }   → git add -- <paths>
  ...
  (guardrail docs/19: ação destrutiva atrás de confirmação).
...
Detalhado em **`docs/21-long-horizon-roadmap.md`** (escrito em 2026-07-09
...
O `docs/21` também contém o **playbook de continuidade** (ritual por
