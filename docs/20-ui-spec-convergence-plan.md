...
`docs/19-architecture-tradeoffs.md`, com gatilho objetivo de reavaliação
(seção "Gatilho de remake" abaixo).

**Por quê gradual e não remake total:**

```text
1. A arquitetura já separou visual de lógica (docs/17): componentes QML
...
2. Big-bang congela entregas por semanas num projeto de uma pessoa cujo
   objetivo declarado é virar daily driver o quanto antes (docs/18).
   Gradual mantém o dogfooding vivo — e dogfooding é o que valida spec.
3. Não existe teste visual automatizado (docs/19, D9): um big-bang
...
R1. UI NOVA nasce conforme spec, sempre. Antes de codar qualquer superfície
    ...
    registrar a interpretação na fatia (docs/18).
R2. Fatia de convergência (C*) é dedicada: não se mistura com feature no
    ...
    entra na fila C.
R3. Cada marco de docs/18 carrega as fatias C mapeadas abaixo. Fatia C
...
## Ordem de convergência (fatias C, amarradas aos marcos de docs/18)
