...
> `docs/specs/` define PARA ONDE (inegociável); `docs-privada/diario/18` sequencia as
...
2. Big-bang congela entregas por semanas num projeto de uma pessoa cujo
   objetivo declarado é virar daily driver o quanto antes (docs-privada/diario/18).
...
R1. UI NOVA nasce conforme spec, sempre. Antes de codar qualquer superfície
    ...
    registrar a interpretação na fatia (docs-privada/diario/18).
R2. Fatia de convergência (C*) é dedicada: não se mistura com feature no
    ...
    entra na fila C.
R3. Cada marco de docs-privada/diario/18 carrega as fatias C mapeadas abaixo. Fatia C
...
## Ordem de convergência (fatias C, amarradas aos marcos de docs-privada/diario/18)
