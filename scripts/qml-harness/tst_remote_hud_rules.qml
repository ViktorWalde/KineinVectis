...
Item {
    ...
    Component.onCompleted: {
        ...
        h = regras.hudFor(estado({ "syncFailed": true, "syncMessage": "connection closed" }));
        ...
        // envio chegou.
        h = regras.hudFor(estado({ "syncFailed": true, "probeOk": true, "probeAgeMs": 10 }));
        check(h.label.indexOf("envio falhou") >= 0, "sonda boa nao apaga envio falho");

        // SONDA VELHA nao vira saude atual.
        h = regras.hudFor(estado({ "probeAgeMs": 5000 }));
        check(h.label.indexOf("verificado agora") >= 0 && h.tone === "ok",
              "medida fresca: " + h.label);
        h = regras.hudFor(estado({ "probeAgeMs": 3600000 }));
        check(h.label.indexOf("verificado") >= 0 && h.label.indexOf("há") >= 0,
              "a idade entra na frase: " + h.label);
        check(h.tone !== "ok", "medida velha perde o tom de ok");

        // TODA afirmacao de saude leva origem e idade no detalhe (§4).
        for (const caso of [{}, { "probeAgeMs": 3600000 }, { "probeOk": false }]) {
            const r = regras.hudFor(estado(caso));
            check(r.detail.indexOf("há") >= 0, "sem idade no detalhe: " + r.detail);
        }
        check(regras.hudFor(estado({})).detail.indexOf("não há conexão aberta") >= 0,
              "diz que nao ha' conexao viva — sondar nao e' estar conectado");

        // Ocupado: o gesto em curso aparece, e manda para Jobs.
        h = regras.hudFor(estado({ "syncing": true, "syncDirection": "push" }));
        check(h.label.indexOf("enviando") >= 0 && h.tone === "ocupado", "push em curso");
        h = regras.hudFor(estado({ "syncing": true, "syncDirection": "pull" }));
        check(h.label.indexOf("puxando") >= 0, "pull em curso");
        check(h.detail.indexOf("Jobs") >= 0, "aponta para Jobs");
        h = regras.hudFor(estado({ "deploying": true }));
        check(h.label.indexOf("deploy") >= 0 && h.tone === "ocupado", "deploy em curso");

        // A idade em palavras.
        check(regras.idade(5000).indexOf("s") >= 0, "segundos");
        check(regras.idade(120000).indexOf("min") >= 0, "minutos");
        check(regras.idade(7200000).indexOf("h") >= 0, "horas");
        check(regras.idade(-1) === "", "idade desconhecida nao inventa numero");
        check(regras.idade(undefined) === "", "idade ausente nao inventa numero");
        ...
        for (const caso of todos) {
            const frase = regras.hudFor(estado(caso));
            check(frase.label.indexOf("sincroniz") < 0,
                  "o HUD voltou a falar de sincronia: " + frase.label);
        }

        // O alvo sempre aparece: "SSH · <alvo> · ...".
        check(regras.hudFor(estado({})).label.indexOf("SSH · pi · ") === 0,
              "prefixo com o alvo");
